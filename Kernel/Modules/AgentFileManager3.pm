# --
# Kernel/Modules/AgentFileManager3.pm - a file download manager
...
package Kernel::Modules::AgentFileManager3;
...
sub new {
    ...
    $Self->{ReadAccessMap} = $Self->{ConfigObject}->Get('FileManager3::ReadAccessMap')
        || { '/' => 'users' };
    $Self->{DeleteAccessMap} = $Self->{ConfigObject}->Get('FileManager3::DeleteAccessMap')
        || { '/' => 'users' };
    $Self->{CreateAccessMap} = $Self->{ConfigObject}->Get('FileManager3::CreateAccessMap')
        || { '/' => 'users' };
...
sub Run {
    ...
    for my $File (@List) {
        ...
        if ( $Self->{ConfigObject}->Get("FileManager3::ListType") ) {
    ...
    $Output .= $Self->{LayoutObject}->Output(
        TemplateFile => 'AgentFileManager3',
        Data         => {
            %Param,
            Location => $Self->{Location},
        },
